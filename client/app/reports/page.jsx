...
import { Loader2, AlertCircle, FileText, Video, Calendar, ArrowRight } from "lucide-react";
    ...
    const fetchAnalyzedSessions = async () => {
      try {
        ...
        if (analyzedSessions.length > 0 && !redirected) {
          ...
          window.location.href = `/reports/${analyzedSessions[0]._id}`;
          return;
        }
        ...
        <main className="flex-1 ml-64 p-10 relative z-10">
          <div className="flex items-center justify-center min-h-[60vh]">
            <div className="glass rounded-2xl p-8 max-w-md text-center shadow-glass">
              ...
              <p className="text-slate-600 mb-6">You haven't analyzed any videos yet. Analyze a video to see detailed reports.</p>
              <button
                onClick={() => window.location.href = "/my-videos"}
                className="btn-gradient text-white px-6 py-2.5 rounded-xl font-medium transition-all shadow-md hover:shadow-lg"
              >
    ...
    <div className="flex min-h-screen relative overflow-hidden">
      ...
      <main className="flex-1 ml-64 p-10 relative z-10">
        <div className="mb-8">
          ...
          <p className="text-slate-600 ml-16">Select a session to view detailed feedback</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sessions.map((session) => {
            const feedback = session.analysis_report?.feedback || session.feedback || {};
            ...
            return (
              <div
                key={session._id}
                onClick={() => window.location.href = `/reports/${session._id}`}
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-lg font-bold text-slate-800">{strengths.length} strengths</p>
                  </div>
                </div>

                <div className="mb-4">
                  <p className="text-sm text-slate-600 mb-2">Analysis Date</p>
